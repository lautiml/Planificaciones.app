...
import services from "../services/gastos.services.js";
...
router.post("/crear", auth.logueado, async (req, res) => {
    try {
        const usuario = await services.crearGasto(req.body);
        res.json(usuario);
    } catch (error) {
        res.status(error.status).json({ error: error.message });
    }
...
router.delete("/eliminar/:id", auth.logueado, async (req, res) => {
    try {
        const response = await services.eliminarGasto(req.params.id);
...
router.put("/modificar/:id", auth.logueado, async (req, res) => {
    try {
        const response = await services.actualizarGasto(req.params.id, req.body, req.userId);
        return res.json(response);
    } catch (error) {
        return res.status(error.status).json({ error: error.message });
    }
