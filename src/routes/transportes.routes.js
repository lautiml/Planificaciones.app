...
import services from "../services/transportes.services.js";
...
router.post("/crear", auth.logueado , async (req, res) => {
    try {
        const usuario = await services.crearTransporte(req.body);
        res.json(usuario);
    } catch (error) {
        res.status(error.status).json({ error: error.message });
    }
...
router.delete("/eliminar/:id", auth.logueado , async (req, res) => {
    try {
        const response = await services.eliminarTransporte(req.params.id);
...
router.put("/modificar/:id", auth.logueado , async (req, res) => {
    try {
        const response = await services.actualizarTransporte(req.params.id, req.body, req.userId);
        return res.json(response);
    } catch (error) {
        return res.status(error.status).json({ error: error.message });
    }
